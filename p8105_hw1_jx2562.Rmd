...
The data frame gives the detailed information about the weather in the early Janurary. The variables in the dataset includes _`r colnames(early_january_weather)`_. The means for _temp_ and _humid_ are `r round(mean(early_january_weather$temp), 2)` and `r round(mean(early_january_weather$humid), 2)`. The standard deviation for _temp_ and _humid_ are `r round(sd(early_january_weather$temp), 2)` and `r round(sd(early_january_weather$humid), 2)`. The first five rows of the dataset are given below.
